...
Constrained by [equality](Language%20Spec.md#comparison-semantics): handles are compared whole, so the encoding must be canonical. No spare or unused bits, and no two bit patterns naming the same entity, or equal handles stop meaning equal entities.
...
Also unsettled, and the part that decides how the language describes itself: **what assigning a `dynamic` field does.** `let s = label.body` either copies the contents — O(n), into somewhere unstated — or aliases them.

Two axes are being conflated wherever this stays open. *Semantics* is what assignment does, copy or share. *Storage* is where the bytes live, inline or runtime-managed. Strings and arrays vary only on the second: [Storage](Language%20Spec.md#storage) calls `dynamic` "transparent for the user", which holds only under copy semantics, since sharing would make a write through one identifier visible through another.

If it resolves toward copying, `dynamic` stays a pure cost annotation and the copied/shared split stays clean — the reference inside the field is representation, not semantics. If it resolves toward sharing, `dynamic` changes a type's semantics, and a `string` becomes copied or shared depending on where it was declared.
...
Components have a fixed layout, so nested `record` fields resolve to static offsets rather than requiring navigation. Worth stating as its own mechanism because several open items lean on it:
...
- It settles that `record` nesting costs nothing at run time, which is what makes records a free abstraction rather than a structural choice.
...
### Handle kinds have different lifetimes · `pending` · `4`

Handle-ness says only that the runtime owns the data and that assignment shares rather than copies. How long a handle stays usable is a property of its kind, and the spec previously stated the entity-and-component rule as though it applied to all handles.

That paragraph has been removed rather than corrected, so the spec now says nothing about how long any handle lasts. It belongs in a lifetime and memory section that does not exist yet, and which needs writing before the rules below can land anywhere.

Two regimes are already visible:

- *Frame-scoped, structurally valid.* Entities and components. Escaping the frame is unspellable, so validity needs no runtime check, and deferred deletion is safe. This is what [handle lifetime mechanics](#data-modeling-and-declaration-syntax) relies on.
- *Long-lived, checked.* A resource handle is held across frames and stored, so validity becomes a runtime question. The sketched answer is the handle state under [memory tiers](#storage-and-memory-layout) — `Pending` / `Ready` / `Failed` / `Dead`, no null, no separate result type — and every use becomes the conditional-binding shape that entry calls the common case.

Open: whether the lifetime is declared per handle type or follows from what the handle points at, how a long-lived handle's validity is spelled at the use site, and what [identity equality](Language%20Spec.md#comparison-semantics) means for a handle that is no longer valid. Nothing here can be written into the spec until resources exist as a language-level concept.

### Handle lifetime mechanics · `pending` · `4`

Settled: every data-carrying type behaves either like a value or a handle ([Data Modeling](Language%20Spec.md#data-modeling)). Entities and components are handles with reference semantics, as classes are in a managed language; the difference is that the runtime owns the memory tightly rather than leaving it in a general arena. The earlier proposal that a component's backing be inferred from provenance is superseded — the distinction is carried by the type, so a write's destination is never a fact about where the identifier came from.
...
- The rule that makes validity structural is that an entity or component handle cannot escape its frame: not stored in a component, not in file state, not held across a boundary. The spec no longer states this — see [handle kinds have different lifetimes](#data-modeling-and-declaration-syntax) — and the enforcement, whether it needs an analysis pass or falls out of the type rules, is not designed either.
...
### Piping construct · `idea` · `2` · `syntax`
...
A bitfield is collection-like, so its membership test takes [`in`](Language%20Spec.md#membership-semantics), and is a mask and a compare. Odin spells this `x in bits` and is the precedent to follow.
...
If they arrive, membership takes [`in`](Language%20Spec.md#membership-semantics) rather than a call, as it does for every other collection. Bit packing qualifies the same way, and the Odin precedent covers both — `key in m`, `x in bits`.
...
[Arrays](Language%20Spec.md#arrays) reads as a fixed count throughout — `integer slots[30]` is described as bounded and inline, and a literal's bound is "the number of elements written". The intent is that a declared bound is a *capacity* and the element count varies at run time, which the spec never says. There is also no way to read a length, and no append or remove.

Surfaced by the comparison pass: [array equality](Language%20Spec.md#comparison-semantics) compares length at run time, which is only correct under the list reading.

### String characters, and what a fixed string can hold · `pending` · `3`

[`length(n)` counts characters](Language%20Spec.md#strings), which follows [Domain over technicism](Design%20Principles.md#domain-over-technicism) but leaves the storage question open: a fixed-length string exists to have a fixed inline size, and a character count does not give one unless the encoding is fixed-width or the field reserves a worst case.

Open: what encoding a string has, whether a fixed-length string may hold non-ASCII at all, and if it may, what `length(16)` costs in bytes. Deferred deliberately — it is a storage question, not a language-surface one, and nothing in the current surface depends on the answer.

### Slices · `pending` · `3`

There is no way to name a sub-range of a string or an array. A slice or span type would cover it, and is what iterating a string should yield — `for c in name` needs a per-character type, and the language has no `char` and does not want one. Until it exists, strings are not iterable, though they are valid [membership](Language%20Spec.md#membership-semantics) operands.

### `ordered` beyond enums · `idea` · `1`

[`ordered`](Language%20Spec.md#enums) is currently an enum annotation, and its check — every field is orderable — is written to compose rather than to be enum-specific. Extending it to `record` types and tuples would make version numbers and dates relationally comparable, and an `ordered` record would become legal as an enum payload with no rule change. No use case has appeared; the keyword is reserved generally so that adding this later is not a breaking change.

[Comparison semantics](Language%20Spec.md#comparison-semantics) therefore states the closed list — numeric types and `ordered` enums — rather than hinting that other types might opt in. The hint belongs here until something can actually carry it.

### Comparing the labels of two runtime enum values · `idea` · `1`

`is` tests a label but needs a literal on the right, so "do `a` and `b` carry the same label, whatever it is" has no spelling. Sorting and grouping by kind are already covered, since [`ordered`](Language%20Spec.md#enums) compares the label first. What is not covered is deduplicating by kind, and whether that is real depends on whether effects are ever held in a list, which is undesigned.
...
Deliberate — evaluating the condition at run time is equally correct, so promising elimination would over-constrain the backend. But it leaves an author with no language-level assurance that a branch behind an absent module costs nothing, which is the shape of implicit cost [No hidden costs](Design%20Principles.md#no-hidden-costs) rules out.
