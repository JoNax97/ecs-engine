...
- No label-only comparison of two runtime values. `is` covers the named case; sorting and grouping by kind fall out of label-first ordering. The gap is deduplication by kind, which has no confirmed use case (`Pending.md#data-modeling-and-declaration-syntax`).
